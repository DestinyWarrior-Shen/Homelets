...
/**
 * This class using RESTful http services to get the available food places data from city of melbourne's API
 */

public class RestClient
{
    //private static final String BASE_URI
    //      = "https://data.melbourne.vic.gov.au/resource/ktrw-apcq.json?$$app_token=AIzaSyB9bfaHtk4lhd3n0A_I2JaBkWkn5swXTHo";

    private static final String BASE_URI
            = "https://data.melbourne.vic.gov.au/resource/ktrw-apcq.json";


    /**
     * implement the http service to get the data.
     */
    public static ArrayList<FoodPlace> collectFoodPlace()
    {
        ...
        String textResult = "";
        ArrayList<FoodPlace> foodPlaceList = new ArrayList<>();
        //Making HTTP request
        try
        {
            ...
            for (int i = 0; i < jsonArray.length(); i++)
            {
                ...
                foodPlace.addAllAttributesToList();
                updateObjectAttributes(foodPlace,foodPlaceList,foodPlace.getAllAttributesToList(),jsonObject);
...
//                            bus_routes,car_parking,bicycle_parking,walking,address_1,address_2,latitude,longitude);
     ...
     */
    public static void updateObjectAttributes(FoodPlace foodPlace,ArrayList<FoodPlace> foodPlacesObjectsList,ArrayList<String> attributeList, JSONObject jsonObject) throws InvocationTargetException, IllegalAccessException
    {
        for (int j=0;j<attributeList.size(); j++)
        {
            ...
            try
            {
                Method method = foodPlace.getClass().getMethod(methodName,String.class);
                method.invoke(foodPlace,attributeValue);
            }
        ...
        if (!(foodPlace.getLatitude().equals("")))
        {
            //foodPlace.setPosition(Double.parseDouble(foodPlace.getLatitude()),Double.parseDouble(foodPlace.getLongitude()));
            foodPlacesObjectsList.add(foodPlace);
        }
    }
